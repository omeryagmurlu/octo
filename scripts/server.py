"""
A server for hosting a Octo model for inference.
...
class HttpServer:
    def __init__(self, paths):
        ...
        for name, path, step in paths:
            self.models[name] = OctoModel.load_pretrained(path, step=step)
    ...
    def reset(self, payload: Dict[Any, Any]):
        model_name = payload.get("model", "octo")
        if "goal" in payload:
    ...
    def sample_actions(self, payload: Dict[Any, Any]):
        try:
            ...
            if not payload.get("ensemble", False): # afaik there was no ensembling on octo, but correct me if im wrong
                print(actions)
...
def main():
    ...
    # name, path, step
    paths = [
        ("octo", "/home/irl-admin/Omer/flower_baselines/octo/outputs/octo_finetune/experiment_20250127_110501", 50000),
    ]
